...
export default function WelcomeCard({ user, onNavigate }) {
  const { t, language } = useLanguage();
  const name = user?.username || 'NovaX';
  const points = user?.loyaltyPoints ?? 8420;
  const balance = 14750;
    ...
    <Panel style={{ flexShrink: 0 }}>
      <div style={{ padding: '14px 18px', position: 'relative' }}>
        ...
        <button
          ...
          {language === 'fa' ? 'مشاهده مسابقات' : 'EXPLORE EVENTS'}
        </button>
      </div>
    </Panel>
  );
